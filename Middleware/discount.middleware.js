...
export const UpdateActiveDiscount = async document => {
    ...
    const updateProduct = await (await ProductModel.find().snapshot()).map(async product => {
        ...
        discountPrice = document.isPercent ? product.price * document?.discount_value / 100 : product.price - document.discount_value;
        await ProductModel.findByIdAndUpdate(product._id, { discount_price: discountPrice })
    })
}